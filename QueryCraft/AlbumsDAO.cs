...
namespace QueryCraft
{
    internal class AlbumsDAO
    {
        ...
        public List<Album> searchTitles(String searchTerm)
        {
            ...
            //define the sql statement to search album
        ...
        public List<JObject> getTracksUsingJoin(int albumID)
        {
            ...
            //define the sql statement to search album
            MySqlCommand command = new MySqlCommand();
            command.CommandText = "SELECT tracks.ID as trackID, albums.Album_TITLE, `track_title`," +
                "`video_url`, `lyrics` FROM  `tracks` JOIN albums ON albums_ID = albums.ID WHERE albums_ID = @albumid";
            command.Parameters.AddWithValue("@albumid", albumID);
            command.Connection = connection;
